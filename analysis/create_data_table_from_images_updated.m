...
tablename = exp_dir + num2str(exp_number) + "_" + num2str(brx_number) + "_from_images_nocalib_imhistmatch_second.mat"; % this is the name of the data table (can be changed and will create a new table) 
...
[centers,radii] = findbeadsbyposition(img1,mask_function,globe_mask); 
...
for i = index_good_data + 1:10:rows
...
%       img = adapthisteq(img,'NumTiles', [10,10],'clipLimit',0.005,'Distribution','rayleigh');
    ...
    % extract centers vector
    [centers,radii] = findbeadsbyposition(img,mask_function,globe_mask);
    disp(i)
