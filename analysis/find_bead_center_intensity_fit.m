function referenced = find_bead_center_intensity_fit(img,center,radi,fit_mod,crop_factor,hshift,vshift,mx)
...
crop_factor = crop_factor; % only half of the radius will be used (so intensity gradient is linear)
cent = center; % estimated coordinate of the center of marker
radi = radi* crop_factor; % radius used to crop the marker into the subimage
hshift = hshift;
vshift = vshift;
img_ref = img;
        ...
        % Keep Top/left hand corner pixel for reference
         x0 = cent(1)-radi+hshift;
          y0 = cent(2)-radi+vshift;
